...
public final class TreeContainer {
	...
	private void init() {
		...
		btnTmp.setIcon(new ImageIcon(TitanWindowDeprecated.class
				.getResource("/res/expand.png")));
		tbarTree.add(btnTmp);
		btnTmp = new JButton("");
		...
		MouseListener ml = new MouseAdapter() {
			public void mousePressed(MouseEvent e) {
				...
				TreePath selPath = treeDSM.getPathForLocation(e.getX(),
						e.getY());
				if (selRow != -1) {
	...
	private void editDSMNodeName(Data dm) {
		...
		while (loopFlag) {
			String input = TaskDialogs.input(null, "Edit DSM name",
					"Current : " + currentDSMName, "");
			...
			if (input == null) {
				...
				if (input.equals("")) {
					TaskDialogs.error(null,
							"DSM name cannot be empty. Enter valid name.", "");
				} else {
					// 중복된 아이템이 존재하는지 확인
					if (this.findNodeByName(input) != null) {
						TaskDialogs.error(null,
								"DSM name must be unique. Try another name.",
								"");
					} else {
						dc.setName(0, input);
	...
	void uiToolbarRename(ActionEvent ae) {
		DefaultMutableTreeNode node = (DefaultMutableTreeNode) treeDSM
	...
	void uiToolbarMoveUp(ActionEvent ae) {
		...
		Data root = (Data) s.invoke("getData");
		dc.moveUp(root.getDataIndex(data));
		s.invoke("reloadDSM");

		DefaultTreeModel model = (DefaultTreeModel) treeDSM.getModel();
		TreePath path = new TreePath(model.getPathToRoot(_findNode(data)
				.getParent()));
		treeDSM.expandPath(path);
		treeDSM.setSelectionPath(new TreePath(model
				.getPathToRoot(_findNode(data))));
	}
	...
	void uiToolbarGroup(ActionEvent ae) {
		...
		for (TreePath p : paths) {
			DefaultMutableTreeNode m = ((DefaultMutableTreeNode) p
					.getLastPathComponent());
	...
	void uiToolbarUnGroup(ActionEvent ae) {
		...
		dc.deleteGroup(data.getDataIndex(this.getSelectedNode()));
	}
	...
	private DefaultMutableTreeNode _findNodeByName(String s) {
		DefaultTreeModel dtm = (DefaultTreeModel) treeDSM.getModel();
		Enumeration<DefaultMutableTreeNode> e = ((DefaultMutableTreeNode) dtm
	...
	public void uiToolbarSort(ActionEvent ae) {
		...
		for (int i = 0; i < parent.getChildCount(); i++) {
			DefaultMutableTreeNode f1 = (DefaultMutableTreeNode) parent
					.getChildAt(i);

			for (int j = 0; j < i; j++) {
				DefaultMutableTreeNode f2 = (DefaultMutableTreeNode) parent
						.getChildAt(j);
				if (f1.toString().compareTo(f2.toString()) < 0) {
					dc.moveUp((Data) f2.getUserObject());
					break;
				}
			}
		}
	...
	public boolean isExpanded(Object item) {
		...
		if (target.getParent() != null) {
			Data dm = (Data) ((DefaultMutableTreeNode) target.getParent())
					.getUserObject();
			if (isExpanded(dm) == true && target.isLeaf())
				return true;
		}
		// System.out.println("look for path, is expanded?" +
		// treeDSM.isExpanded(path));
	...
	public boolean isVisible(Object item) {
		...
		// ((Data)target.getUserObject()).getName() + ", vis : " +
		// treeDSM.isVisible(path) + ", expand? " + treeDSM.isExpanded(path));
	...
	public int getVisibleNodeCount(DefaultMutableTreeNode parent) {
		...
		while (e.hasMoreElements()) {
			DefaultMutableTreeNode node = e.nextElement();
			if (isVisible(node.getUserObject()) == false) {
				// System.out.println("접혀진 노드 : " +
				// ((Data)node.getUserObject()).getName());
			} else {
				// 펼쳐진 모듈이면 카운트 안함
				// 카운트 조건 : 보여지면서 펼쳐진 것이 아닌 것
				if (isExpanded(node.getUserObject()) == false || node.isLeaf()) {
					// System.out.println("펼쳐진 노드 : " +
					// ((Data)node.getUserObject()).getName());
					count++;
				}
			}
