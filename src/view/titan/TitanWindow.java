...
public class TitanWindow implements ActionListener{
   ...
   private Data getData(){
      return this.currentDataController.getData();
   }
   
   void setData(Data d){
      this.currentDataController.data = d;
   }
         ...
         if(value != null){
            try{
               ...
               for(int i = 0; i < rowCount; i++){
                  dc.addItem(this.currentDataController.getData(), currentDataController.getData().getDataIndex(this.treeContainer.getSelectedNode()), _entity + (i + 1));
               }
   ...
   void _buildTable(TableContainer tbl, Data parent){
      for(int i = 0; i < parent.getChildLength(); i++){
         ...
         if(inner.getChildLength() > 0){
            ...
            for(int j = 0; j < currentDataController.getData().getItemCount(); j++){
               //대각성분은 .으로 표시
               if(depth == j){
                  vc.add(".");
               }else{
   ...
   void buildTable(TableContainer tbl, Data parent){
      ...
      for(int i = 0; i < dts.length; i++){
         dtsIdxs[i] = currentDataController.getData().getDataIndex(dts[i]);
         System.out.println(dtsIdxs[i]);
      }
      
      boolean maps[][] = currentDataController.getDependArray(dtsIdxs);
   ...
   public void loadDSMFromData(){
      ...
      //트리의 모든 요소 삭제
      tc.setRoot(currentDataController);
            
      //차일드 아이템 설정
      Object root = tc.getRoot();
      
      buildTree(tc, currentDataController.getData());
      ...
      //의존도 정보를 셀에 표시
      depth = 0;
      buildTable(tblContainer, currentDataController.getData());
   }
   ...
   private void uiMnuOpenDSM(ActionEvent ae){
      ...
      if(jfc.showOpenDialog(frame) == JFileChooser.APPROVE_OPTION){
         ...
         currentDataController.loadDsm(dsmFile);
         loadDSMFromData();
      }else{
         dsmFile = null;
      }
   ...
   void uiMnuNewClustering(ActionEvent ae){
      this.currentDataController.loadDsm(dsmFile);
      loadDSMFromData();
   }
   ...
   void uiMnuLoadClustering(ActionEvent ae){
      ...
      if(jfc.showOpenDialog(frame) == JFileChooser.APPROVE_OPTION){
         ...
         currentDataController.loadDsmClsx(currentDataController.getData(), jfc.getSelectedFile());
         loadDSMFromData();
      }else{
         clsxFile = null;
      }
