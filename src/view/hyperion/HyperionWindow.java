...
/**
 * 타이탄 윈도우 이 윈도우에 DSM과 클러스터링 정보를 표시하는 트리, 테이블이 추가되며 이들을 제어할 수 있는 명령 도구들이 배치됩니다.
 * 
 * @author Seok
 *
 */
public class HyperionWindow {
	...
	private void initMenuBar() {
		...
		mnuTmp.add(UIHelper.buildMenuItem("Save DSM", evtObj, KeyEvent.VK_S,
				InputEvent.CTRL_MASK));
		mnuTmp.add(UIHelper.buildMenuItem("Save DSM As...", evtObj,
				KeyEvent.VK_S, InputEvent.CTRL_MASK | InputEvent.SHIFT_MASK));
		mnuTmp.add(new JSeparator());
		...
		mnuTmp.add(UIHelper.buildMenuItem("Partitioning", evtObj,
				KeyEvent.VK_F8, 0));
		mnuTmp.add(new JSeparator());
	...
	private void uiMnuNewDSM(ActionEvent ae) {
		...
		if (selNode == null) {
			UIHelper.BuildWarnDlg("Error",
					"Select node first please. Try Again", "");
			return;
		}
			...
			if (value != null) {
				try {
					...
					for (int i = 0; i < rowCount; i++) {
						dcon.addItem(
								dcon.getRootData().getDataIndex(
										treeContainer.getSelectedNode()),
								"entity_" + (++entityIdxCounter));
					}
	...
	/**
	 * 클러스터링 초기화<br/>
	 * 지금까지 수행한 DSM과 클러스터링 데이터를 초기화하고 DSM을 새로 불러온다. 이렇게하면 새로운 클러스터링을 만드는 것과 같다.
	 * 
	 * @param ae
	 */
	...
	void uiMnuLoadClustering(ActionEvent ae) {
		if (dcon.getRootData().getChildLength() == 0) {
			UIHelper.BuildWarnDlg(
					"Error",
					"Cannot load clustering. Make sure load dsm file before load clustering.",
					"Reason : DSM not loaded.");
			return;
		}
		FileNameExtensionFilter flt = new FileNameExtensionFilter(
	...
	void uiMnuSaveClustering(ActionEvent ae) {
		dcon.saveClsx(dsmFile.toString(), dcon.getRootData());
	}
	...
	void uiMnuSaveClusteringAs(ActionEvent ae) {
		...
		if (jfc.showSaveDialog(frame) == JFileChooser.APPROVE_OPTION) {
			...
			dcon.saveClsx(clsxAs.toString(), dcon.getRootData());
		}
	}
	...
	void uiMnuSaveDSM(ActionEvent ae) {
		dcon.saveDSM(dsmFile.toString() + ".dsm", dcon.getRootData());
	}
	 ...
	 * @param ae
	 */
	void uiMnuSaveDSMAs(ActionEvent ae) {
		FileNameExtensionFilter flt = new FileNameExtensionFilter(
				"DSM File(*.dsm)", "dsm");
		JFileChooser jfc = new JFileChooser();
		jfc.setDialogTitle("Save Clustering File As...");
		...
		if (jfc.showSaveDialog(frame) == JFileChooser.APPROVE_OPTION) {
			File clsxAs = jfc.getSelectedFile();
			dcon.saveDSM(clsxAs.toString() + ".dsm", dcon.getRootData());
		}
	}
	...
	void uiMnuAbout(ActionEvent ae) {
		TaskDialogs
				.inform(null, "2015 Human ICT Software Engineering, Team 9",
						"남궁석-20101518\r\n노효빈-20101522\r\n김대훈-20114207\r\n하진-20135166\r\n");
	}
	...
	void buildTable(TableContainer tbl, Data parent) {
		try {
			...
			if (isClusterLoad != false) {
				...
				// 보여지는 노드를 가져옴
				DefaultMutableTreeNode[] nodes = treeContainer
						.getVisibleTreeAllNodes();
				...
				wholeMatrix = removeMatrix(wholeMatrix,
						dtsIdxs[indexLength - 1] + 1, wholeMatrix.length);
				for (int d = 0; d < indexLength - 1; d++) {
					wholeMatrix = removeMatrix(wholeMatrix, dtsIdxs[indexLength
							- d - 2] + 1, dtsIdxs[indexLength - d - 1] - 1);
				}
	...
	private int[][] removeMatrix(int[][] map, int start, int end) {
		// int[][] ret = new
		// int[map.length-(end-start+1)][map.length-(end-start+1)];
