...
public class DataDsmTest {
	...
	public void moveUpTest() {
		String name = dataController.data.getChild(1).getName();
		// moveUp의 parameter는 root를 포함한 1부터 시작하기 때문에 2를 넣으면 root, 다음, 다음이 선택된다.
		// 즉 구조상으로 child의 1번이 올라가는셈.
		dataController.moveUp(2);
		assertThat(name, is(dataController.data.getChild(0).getName()));
	}

	@Test
	public void moveDownTest() {
		String name = dataController.data.getChild(0).getName();
		dataController.moveDown(1);
		assertThat(name, is(dataController.data.getChild(1).getName()));
	}

	@Test
	public void changeNodeNameTest() {
		dataController.changeNodeName(1, "Test");
		assertThat("Test", is(dataController.data.getChild(0).getName()));
	}
	...
	public void addDsmTest() {
		...
		assertThat(name, is(dataController.data.getChild(66).getName()));
	}
	...
	public void duplicateTest() {
		...
		String name = dataController.data.getChild(3).getName();
		data = new Data(dataController.duplicate(4));
		assertThat(name, is(data.getChild(0).getName()));
	}
